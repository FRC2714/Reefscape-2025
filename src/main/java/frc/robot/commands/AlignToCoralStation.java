...
public class AlignToCoralStation extends Command {
  ...
  public AlignToCoralStation(DriveSubsystem m_drivetrain, Limelight m_limelight) {
    ...
    xController.setSetpoint(0.1); // TODO
  ...
  public void execute() {
    if ((m_limelight.isTargetVisible())) { // if can only see left, then do whatever we did before
      ...
      m_drivetrain.drive(-xController.calculate(m_limelight.getDistanceToGoalMeters()),
          m_limelight.getDistanceToGoalMeters() < 0.4 ? yController.calculate(m_limelight.getXOffsetRadians())
              : 0,
  ...
  private void updateThetaControllerSetpoint(int targetID) {
    switch (targetID) {
      case 2, 12 -> thetaController.setSetpoint(180 + 54.011);
      case 1, 13 -> thetaController.setSetpoint(180 - 54.011);
    }
